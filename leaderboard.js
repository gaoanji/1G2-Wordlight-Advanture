...
(() => {
  'use strict';
  const ENDPOINT='https://script.google.com/macros/s/AKfycbzq43BIVDWUDh7G9zwZeaiy9e4nEbN3J0db-0OIcml4Hu589Yoz0iyJPyCGej50RD2h/exec';
  const KEY='wordlight_cloud_queue_v1', IDKEY='wordlight_cloud_ids_v1';
  ...
  function flushCards(){
    ...
    try{localStorage.removeItem('wordlight_pending_cards_v1')}catch{}
  }
  function card(word){
    ...
    try{localStorage.setItem('wordlight_pending_cards_v1',JSON.stringify({eventId:session.pendingId,profile:session.profile,runId:session.runId,stage:'cards',words:[...session.words],seconds:Math.min(14400,Math.floor(session.seconds))}))}catch{storageError=true;updateStatus()}
  ...
  const recovered=read('wordlight_pending_cards_v1',null);
  if(recovered?.words?.length){enqueue(recovered);try{localStorage.removeItem('wordlight_pending_cards_v1')}catch{}}
